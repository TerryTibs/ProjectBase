# --- START OF FILE project/agents/dqn_rainbow_lstm_agent.py ---
...
class DQNRainbowLSTMAgent(BaseAgent):
    """ Rainbow DQN Agent with LSTM (TF) - Uses network builder """
    def __init__(self, game,
                 lr=0.00005, gamma=0.99, batch_size=64, # 64 before
                 memory_size=1000000, target_update_freq=10000, # In frames mem 100000 before
                 lstm_units=32, # LSTM specific param
                 per_alpha=0.6, per_beta_start=0.4, per_beta_frames=1000000,
                 noisy_std_init=0.5,
                 shared_hidden_units=(64, 64, 64, 64, 64, 64, 64, 64), activation='relu', # Network params 64 64 before
                 model_path=None,
        ...
        self.gamma = gamma
        self.learning_rate = lr # Store learning rate
        ...
        # Build Networks with LSTM and Noisy layers
        self.lstm_input_shape = (1, self.state_size) # (timesteps, features)
        self.model = build_lstm_dueling_dqn(
            input_shape=self.lstm_input_shape,
            num_actions=self.action_size,
            lstm_units=self.lstm_units,
            noisy=True, # Use NoisyLinear
            noisy_std_init=self.noisy_std_init,
            shared_hidden_units=shared_hidden_units,
            activation=activation,
            name="DQNRainbowLSTM_Online"
        )
    ...
    def get_action(self, state):
        """Reshapes state, predicts action using noisy network."""
        # Reshape flat state to (1, 1, state_size) for LSTM
    ...
    def _train_step_tf(self, states, actions, rewards, next_states, dones, weights):
        ...
        # Reshape flat states for LSTM input: (batch, 1, features)
        states_lstm = tf.reshape(states, [-1, 1, self.state_size])
        next_states_lstm = tf.reshape(next_states, [-1, 1, self.state_size])

        # Double DQN Target Calculation
        online_next_q = self.model(next_states_lstm, training=True) # Use noise for action selection
        next_actions = tf.argmax(online_next_q, axis=1, output_type=tf.int32)
        target_next_q = self.target_model(next_states_lstm, training=False) # Target without noise
        ...
        with tf.GradientTape() as tape:
            current_q_all = self.model(states_lstm, training=True) # Online model needs noise
    ...
    def learn(self):
        ...
        if batch is None:
            self._log(logging.WARNING, "Rainbow LSTM sampling failed.")
            return None

        # Batch contains flat states
        ...
        # Convert to tensors (reshaping happens inside _train_step_tf)
    ...
    def update_target_model(self):
        ...
        self._log(logging.INFO, f"DQNRainbowLSTMAgent updating target model at frame {self.frame_idx}")
    ...
    def load(self, name_base):
        ...
        # Custom objects needed for Dueling + Noisy + LSTM
    ...
    def save(self, name_base):
        ...
        model_file = name_base + ".keras"
        # Saving usually handles custom layers okay if they are registered or provided at load time
        self._save_keras_model(self.model, model_file) # Uses helper which logs

# --- END OF FILE project/agents/dqn_rainbow_lstm_agent.py ---
