...
#
# VERSION 28.2 (This version): Corrected NameError
# - Fixed a NameError typo in the main training loop where 'next_symbolic_state'
#   was incorrectly called 'next_sym_state'.
...
class HRL_KB_Agent:
    def __init__(self, game: SimpleSnakeGame):
        self.game = game; self.kb = SymbolicKnowledgeBase(); self.options = self._setup_options(); self.meta_controller = self._setup_meta_controller()
    ...
    def _setup_options(self):
        ...
        for name in ["GoToApple", "AvoidTrap"]:
            options[name] = {'name': name, 'online_network': build_dqn_network(STATE_SIZE, ACTION_SIZE, (64,32), f"{name}_Online"), 'target_network': build_dqn_network(STATE_SIZE, ACTION_SIZE, (64,32), f"{name}_Target"), 'replay_buffer': PrioritizedReplayBuffer(2000), 'batch_size': 32}
            options[name]['target_network'].set_weights(options[name]['online_network'].get_weights())
        return options
    def _setup_meta_controller(self):
        mc = {"option_names": list(self.options.keys()), "option_indices": {name: i for i, name in enumerate(self.options.keys())}, "online_network": build_dqn_network(STATE_SIZE, len(self.options), (128,64), "Meta_Online"), "target_network": build_dqn_network(STATE_SIZE, len(self.options), (128,64), "Meta_Target"), "replay_buffer": PrioritizedReplayBuffer(5000), "batch_size": 16, "gamma_meta": 0.99}
    ...
    def get_action(self, symbolic_state: dict) -> int:
        ...
        if self.active_option:
            ...
            if terminated:
                ...
                self.meta_controller['replay_buffer'].add(exp); self.active_option = None
    ...
    def learn_components(self, prev_sym_state, action, reward, next_sym_state, done, global_step):
        if self.active_option:
            ...
            opt['replay_buffer'].add((self._vectorize_state(prev_sym_state), action, intrinsic_reward, self._vectorize_state(next_sym_state), done))
        ...
        if done and len((mc := self.meta_controller)['replay_buffer']) >= mc['batch_size']: self._train_model(mc, mc['gamma_meta'])
    ...
    def save_models(self, path="models"):
        ...
        self.meta_controller['online_network'].save(os.path.join(path, "meta_controller_q_net.keras"))
...
def main(args):
    ...
    for episode in range(1, args.episodes + 1):
        ...
        while not done:
            ...
            # This is the line with the variable that caused the error
            next_symbolic_state, reward, done = env.step(action)
            total_steps += 1
            
            # CORRECTED: Use the correct variable name 'next_symbolic_state'
