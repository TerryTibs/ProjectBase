# standalone_learning_hrl_kb_agent_v19_tuned.py
# A Proof of Concept for a HRL Agent with learning components.
# VERSION 19: Hyperparameter Tuning - Slower epsilon decay, lower learning rate,
# larger networks, and increased option gamma for better learning.
...
# TUNING: Lowered learning rate for more stable updates.
def build_dqn_network(input_size, output_size, hidden_units=(64, 32), name="DQN", learning_rate=0.00025):
...
class HRL_KB_Agent:
    ...
    def _setup_options(self):
        options = {}
        # TUNING: Increased network size for options
        options["GoToApple"] = {
            "name": "GoToApple",
            "policy_network": build_dqn_network(STATE_SIZE, ACTION_SIZE, hidden_units=(64, 32), name="GoToApple_Policy"),
            "replay_buffer": deque(maxlen=2000), "batch_size": 32,
            "epsilon": 0.9, "epsilon_decay": 0.9999, "epsilon_min": 0.05 # Slower decay
        }
        options["AvoidTrap"] = {
            "name": "AvoidTrap",
            "policy_network": build_dqn_network(STATE_SIZE, ACTION_SIZE, hidden_units=(64, 32), name="AvoidTrap_Policy"),
            "replay_buffer": deque(maxlen=2000), "batch_size": 32,
            "epsilon": 0.9, "epsilon_decay": 0.9999, "epsilon_min": 0.05 # Slower decay
        }
        return options
    ...
    def _setup_meta_controller(self):
        mc = {
            ...
            # TUNING: Increased network size for meta-controller
            "meta_q_network": build_dqn_network(STATE_SIZE, len(self.options), (128,64), "MetaController_QNet"),
            "replay_buffer": deque(maxlen=5000), "batch_size": 16,
            "epsilon": 1.0, "epsilon_decay": 0.999, "epsilon_min": 0.1, "gamma_meta": 0.99 # Slower decay, start at 1.0
        }
        return mc
...
# --- Main Runner ---
def main(args):
    logger.info(f"--- Initializing LEARNING HRL-KB PoC (v19 Tuned) ---")
    ...
    for episode in range(1, args.episodes + 1):
        ...
        logger.info(f"--- Episode {episode} END --- Score: {env.score}, Total Reward: {episode_reward:.2f}, Meta Eps: {agent.meta_controller['epsilon']:.3f}\n")
