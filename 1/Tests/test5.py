...
#
# VERSION 26 (This version): Added Color Perception
# - The game now assigns random colors to apples and traps from predefined palettes.
# - The symbolic state includes one-hot encoded features for the color of the
#   apple and the trap, allowing the agent to "see" and differentiate them.
...
# Dynamically generate feature names for colors
...
SYMBOLIC_FEATURE_NAMES = sorted([
    ...
    'danger_forward', 'danger_left_rel', 'danger_right_rel',
    'tail_dx', 'tail_dy',
    ...
    'apple_vector_x',
    'apple_vector_y'
] + apple_color_features + trap_color_features) # Add the new color features
...
class SimpleSnakeGame:
    def __init__(self, render=True, grid_size=10):
        ...
        # --- NEW: Store color definitions in the instance ---
        self.apple_colors = APPLE_COLORS
        self.trap_colors = TRAP_COLORS
        self.apple_color_names = APPLE_COLOR_NAMES
        self.trap_color_names = TRAP_COLOR_NAMES
    ...
    def reset(self):
        ...
        # --- NEW: Assign colors on reset ---
        self.apple_pos = self._place_item()
        self.apple_color_name = random.choice(self.apple_color_names)
        self.trap_pos = self._place_item()
        self.trap_color_name = random.choice(self.trap_color_names)
        ...
        if self.render_mode: logger.info(f"Env Reset: Player@{self.snake[0]}, Apple@{self.apple_pos} ({self.apple_color_name}), Trap@{self.trap_pos} ({self.trap_color_name})")
    ...
    def get_symbolic_state(self) -> Dict:
        ...
        dir_left_rel = (dy, -dx); dir_right_rel = (-dy, dx)
        ...
        state['snake_length_normalized'] = len(self.snake) / max_len if max_len > 0 else 0.0
        
        head_x, head_y = head
        state['is_on_edge'] = 1.0 if (head_x == 0 or head_x == self.grid_size - 1 or head_y == 0 or head_y == self.grid_size - 1) else 0.0
        
        state['is_apple_aligned_x'] = 1.0 if state['apple_dx'] == 0 else 0.0
        state['is_apple_aligned_y'] = 1.0 if state['apple_dy'] == 0 else 0.0
        ...
        # --- NEW: One-hot encode apple and trap colors ---
        for name in self.apple_color_names:
            state[f'apple_color_{name}'] = 1.0 if name == self.apple_color_name else 0.0
        for name in self.trap_color_names:
            state[f'trap_color_{name}'] = 1.0 if name == self.trap_color_name else 0.0
    ...
    def step(self, action):
        ...
        if new_head == self.apple_pos:
            self.score += 1; reward = 10.0
            # --- NEW: Respawn items with new random colors ---
            self.apple_pos = self._place_item()
            self.apple_color_name = random.choice(self.apple_color_names)
            self.trap_pos = self._place_item()
            self.trap_color_name = random.choice(self.trap_color_names)
    ...
    def render(self):
        ...
        # --- NEW: Use dynamic colors for rendering ---
        apple_rgb = self.apple_colors[self.apple_color_name]
        trap_rgb = self.trap_colors[self.trap_color_name]
...
class HRL_KB_Agent:
    ...
    def learn_components(self, prev_sym_state, action, reward, next_sym_state, done, global_step):
        if self.active_option:
            ...
            if opt['name'] == "GoToApple":
                ...
                intrinsic_reward = progress * 2.0 if progress > 0 else -5.0
    ...
    def load_models(self, path="models"):
        try:
            ...
            logger.warning(f"Could not load saved models from '{path}'. This is expected if the state size has changed. Starting from scratch. Error: {e}")
